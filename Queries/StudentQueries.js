...
const GET_ATTENDANCE_BY_DATE =
  "select distinct a.attendance_date, s.id, s.name as student_name, c.name as class_name, ss.total, ss.consecutive_classes, ss.streak_of_four from students as s, student_class as sc, student_stats as ss, classes as c, attendance as a where sc.student_id = s.id and c.id = sc.class_id and ss.student_id = s.id and s.id = a.student_id and a.attendance_date = $1";

const GET_ATTENDANCE =
  "select distinct s.id,  s.name as student_name, c.name as class_name, ss.total, ss.consecutive_classes, ss.streak_of_four from students as s, student_class as sc, student_stats as ss, classes as c where sc.student_id = s.id and c.id = sc.class_id and ss.student_id = s.id";
